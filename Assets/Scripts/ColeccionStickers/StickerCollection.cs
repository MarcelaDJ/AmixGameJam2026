...
/// <summary>
/// Pure logic of the sticker collection, modeled as a fixed-size
...
public class StickerCollection
{
    ...
    /// <summary>Fired whenever the collection changes (to refresh UI, etc.)</summary>
    ...
    /// <summary>How many empty slots are left.</summary>
    ...
    /// <summary>Returns every non-empty slot belonging to a given category.</summary>
    ...
    /// <summary>Checks whether a specific sticker is owned at all.</summary>
    public bool Owns(StickerData sticker)
    {
        return slots.Any(s => !s.IsEmpty && s.data == sticker);
    }

    /// <summary>Total points across the whole collection (points x count).</summary>
    public int TotalPoints()
    {
        return slots.Where(s => !s.IsEmpty).Sum(s => s.data.points * s.count);
    }

    /// <summary>Number of distinct sticker types currently occupying a slot.</summary>
